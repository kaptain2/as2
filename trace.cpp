...
bool Triangle::intersect(const Ray &r, double t0, double t1, HitRecord &hr) const {
    ...
    beta = (r.e[0] - a[0] - (c[0]-a[0])*(r.d[1]* (r.e[0]-a[0])-r.d[0]*(r.e[1]-a[1]))/o0 - r.d[0] * (r.e[1]-a[1])*(c[1]-a[1])-(r.e[2]-a[2])*(c[2]-a[2])/o1)*o0*o1/ ((b[0]-a[0])*o0*o1 + o1* (r.d[1]*(b[0]-a[0])-r.d[0]*(b[1]-a[1]))* (c[0]-a[0]) + o0* ((c[2]-a[2])*(b[1]-a[1])-(c[1]-a[1])*(b[2]-a[2]))*r.d[0]);
